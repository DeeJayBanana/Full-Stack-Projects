...
<body>
    ...
    <main>
        <div class="atlas">
            <?php
                if(isset($_POST['nazwa']) && (isset($_POST['opis']))){
                    ...
                    $nazwa = $_POST['nazwa'];
                    $obrazek = "./grafika/".$_FILES['obrazek']['name'];
                    $opis= $_POST['opis'];

                    $zap = "insert into zwierzeta values (null, '$nazwa', '$obrazek', '$opis')";
                    $row = mysqli_query($polacz, $zap);

                    function zapisz_plik(){
                        $plik = $_FILES['obrazek']['name'];
                        $lokalizacja = './grafika/'.$plik;
                        if(!move_uploaded_file($_FILES['obrazek']['tmp_name'], $lokalizacja)){
                            echo ('Problem: Nie udało się skopiować pliko do katalogu');
                        }
                    }
                    zapisz_plik();
                }
        ...
        <form enctype="multipart/form-data" action="dodaj.php" method="POST">
            ...
            <textarea name="opis" rows="6" placeholder="Opis"></textarea>
